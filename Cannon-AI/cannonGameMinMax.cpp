...
public:
    ...
    CannonGame(int player, int n, int m){
        ...
        if(player == 1){
            ...
            coefficients.push_back(-20); //number of soldiers that can be attacked, player 1
            coefficients.push_back(5); //number of soldier that can be attacked, player 2

            coefficients.push_back(36); // number of soldier, 1
            coefficients.push_back(-25); //number of soldier, 2

            coefficients.push_back(500); // number of townhall, 1
            coefficients.push_back(-450); // number of townhall, 2

            coefficients.push_back(20); // number of cannon, 1
            coefficients.push_back(-20); //numberOfCannon, 2

            coefficients.push_back(-1); //closenessFactor. 1
            coefficients.push_back(2); //closenessFactor 2

            coefficients.push_back(-100); // numberOfTownHallCanBeAttacked, 1
            coefficients.push_back(20); // numberOfTownHallCanBeAttacked, 2
        }

        if(player == 2){

            coefficients.push_back(5); //number of soldiers that can be attacked, player 1
            coefficients.push_back(-20); //number of soldier that can be attacked, player 2

            coefficients.push_back(-25); // number of soldier, 1
            coefficients.push_back(36); //number of soldier, 2

            coefficients.push_back(-450); // number of townhall, 1
            coefficients.push_back(500); // number of townhall, 2

            coefficients.push_back(-20); // number of cannon, 1
            coefficients.push_back(20); //numberOfCannon, 2

            coefficients.push_back(2); //closenessFactor. 1
            coefficients.push_back(-1); //closenessFactor 2

            coefficients.push_back(20); // numberOfTownHallCanBeAttacked, 1
            coefficients.push_back(-100); // numberOfTownHallCanBeAttacked, 2
        }
    ...
    int evaluationFunction(vector<vector<int> > myState){
        ...
        int numberOfActiveCannonPlayer1 = 0, numberOfActiveCannonPlayer2 = 0;
        ...
        for(int i=0;i<n;i++){
            for(int j=0;j<m;j++){
                ...
                if(tempPlayer == 1){
                    ...
                    if(canCannonFire(i, j, 1, myState)){
                    	numberOfActiveCannonPlayer1++;
                    }
                }
                if(tempPlayer == 2){
                    ...
                    if(canCannonFire(i, j, 2, myState)){
                    	numberOfActiveCannonPlayer2++;
                    }
                    // }
                }
                if(tempPlayer == 10){
        ...
        answer = answer + coefficients[4]*numberOfTownHallPlayer1 + coefficients[5]*numberOfTownHallPlayer2 + coefficients[6]*numberOfActiveCannonPlayer1 + coefficients[7]*numberOfActiveCannonPlayer2;
    ...
    bool canCannonFire( int i, int j, int player, vector<vector<int> > myState){
    	int n = myState.size();
    	int m = myState[0].size();
	        ...
	        if(player == 2){
	            if(j-2 >= 0){
	                if(myState[i][j-1] == 2 && myState[i][j-2] == 2){
	                    if(j+2 < m){
	                        if((myState[i][j+2] == otherPlayer || myState[i][j+2] == otherPlayer*10 )&& myState[i][j+1] == 0){
	                            return true;
	                        }
	                    }
	                    if(j+3 < m){
	                        if((myState[i][j+3] == otherPlayer || myState[i][j+3] == otherPlayer*10) && myState[i][j+1] == 0){
	                        }
	                    }
	                    if(j-4 >= 0){
	                        if((myState[i][j-4] == otherPlayer || myState[i][j-4] == otherPlayer*10) && myState[i][j-3] == 0){
	                            return true;
	                        }
	                    }
	                    if(j-5 >= 0){
	                        if((myState[i][j-5] == otherPlayer || myState[i][j-5] == otherPlayer) && myState[i][j-3] == 0){
	                            return true;
	                        }
	                    }
	                }
	                if(i-2 >= 0 && j-2 >= 0){
	                    if(myState[i-1][j-1] == 2 && myState[i-2][j-2] == 2){
	                        if(j+2 < m && i+2 < n){
	                            if((myState[i+2][j+2] == otherPlayer || myState[i+2][j+2] == otherPlayer) && myState[i+1][j+1] == 0){
	                                return true;
	                            }
	                        }
	                        // cout<<"sona13"<<endl;
	                        if(j+3 < m && i+3 < n){
	                            if((myState[i+3][j+3] == otherPlayer || myState[i+3][j+3] == otherPlayer*10) && myState[i+1][j+1] == 0){
	                                return true;
	                            }
	                        }
	                        // cout<<"sona14"<<endl;
	                        if(j-4 >= 0 && i-4 >= 0){
	                            if((myState[i-4][j-4] == otherPlayer || myState[i-4][j-4] == player*10) && myState[i-3][j-3] == 0){
	                                return true;
	                            }
	                        }

	                        if(j-5 >= 0 && i-5 >= 0){
	                            if((myState[i-5][j-5] == otherPlayer || myState[i-5][j-5] == otherPlayer*10) && myState[i-3][j-3] == 0){
	                                return true;
	                            }
	                        }
	                    }
	                ...
	                if(i+2 < n && j-2 >= 0){
	                    if(myState[i+1][j-1] == 2 && myState[i+2][j-2] == 2){
	                        ...
	                        if(j+2 < m && i-2 >= 0){
	                            if((myState[i-2][j+2] == otherPlayer || myState[i-2][j+2] == otherPlayer*10) && myState[i-1][j+1] == 0){
	                                return true;
	                            }
	                        }
	                        if(j+3 < m && i-3 >= 0){
	                        ...
	                        if(i+4<n && j-4 >= 0){
	                            if((myState[i+4][j-4] == otherPlayer || myState[i+4][j-4] == otherPlayer*10) && myState[i+3][j-3] == 0){
	                                return true;
	                            }
	                        }
	                        if(j-5 >= 0 && i+5 < n){
	            ...
	            if(i-2 >= 0){
	                if(myState[i-1][j] == 2 && myState[i-2][j] == 2){
	                    ...
	                    if(i+2 < n){
	                        if((myState[i+2][j] == otherPlayer || myState[i+2][j] == otherPlayer*10) && myState[i+1][j] == 0){
	                            return true;
	                        }
	                    }
	                    if(i+3 < n){
	                        if((myState[i+3][j] == otherPlayer || myState[i+3][j] == otherPlayer*10) && myState[i+1][j] == 0){
	                            return true;
	                        }
	                    }
	                    if(i-4 >= 0){
	                        if((myState[i-4][j] == otherPlayer || myState[i-4][j] == otherPlayer*10) && myState[i-3][j] == 0){
	                            return true;
	                        }                                   
	                    }
	                    if(i-5 >= 0){
	                        if((myState[i-5][j] == otherPlayer || myState[i-5][j] == otherPlayer*10) && myState[i-3][j] == 0){
	                            return true;
	                        }
	                    }
	                }
	        	...
	        	if(j+2 < m){
	                if(myState[i][j+1] == 1 && myState[i][j+2] == 1){
	                    ...
	                    if(j+5 < m){
	                        if((myState[i][j+5] == otherPlayer || myState[i][j+5] == otherPlayer*10) && myState[i][j+3] == 0){
	                            return true;
	                        }
	                    }
	                }
    ...
    if(identity == 1){
    	if(n == 8 && m == 8){
	        while(timed < time){
	            ...
	            string s = "";
	            if(moveCount == 0){
		            game->nextMove(s, game->board, 3, INT_MIN, INT_MAX, player);
	            }
	            else if(timed < 75){
	            	...
	            	game->nextMove(s, game->board, 1, INT_MIN, INT_MAX, player);
	            }
	            cout<<s<<endl;

	            stop = high_resolution_clock::now();
	            duration = duration_cast<milliseconds>(stop - start);
	            timed += (float)duration.count() / 1000;
